...
function Server(info) {
	...
	this._server.sockets.on('connection', function(socket) {
		...
		socket.on('data', function(data) {
			...
			if (data.name =="auth" || !self.info.auth || client.auth){
				self.event.emit("log", client, data);
		    	self.event.emit(data.name, client, data.data);
		    } else {
		...
		socket.on('clientConnect', function(info){
			...
			if (!(clientName in self.clients)) {
				...
				if (self.clients[clientName].socket.id != socket.id) {
